 ...
 * selected.
 * 
 * @param drive_setup The style of drive,f such as TANK_TWO_ROTATION.
...
void Drive::control_arcade(){
  ...
  float turn = deadband(controller(primary).Axis3.value(), 3);
  DriveL.spin(fwd, to_volt(throttle+turn), volt);
  DriveR.spin(fwd, to_volt(throttle-turn), volt);
  double xAxisInput = Controller1.Axis3.value();
  double yAxisInput = Controller1.Axis1.value();
  if((1)){
    xAxisInput = xAxisInput*xAxisInput*xAxisInput/1000;
    yAxisInput = yAxisInput*yAxisInput*yAxisInput/1000;
  }
  double pwrL = xAxisInput+yAxisInput;
  double pwrR = xAxisInput-yAxisInput;
